...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function platformsearch() {//平台交易订单查询
        ...
        $data = array("out_trade_no" => $out_trade_no, "Transaction_id" => $transactionid, "SystemUserSysNo" => session('SysNO'));
    ...
    private function AliQueryOrder($out_trade_no) {
        ...
        $data = array("out_trade_no" => $out_trade_no, "SystemUserSysNo" => session('SysNO')
...
//        exit;
    ...
    public function WXSupplyOrder() {
        $out_trade_no = trim($_POST['out_trade_no']);
        $trade_state = $this->WxQueryOrder($out_trade_no);//微信查询
        $trade_count = $this->StaffWxQueryOrder($out_trade_no);//订单退款查询
        if ($trade_count == 0) {
            if ($trade_state == 'SUCCESS') {
                $ActStatus = $this->AddWxOrder($out_trade_no);
                if ($ActStatus) {
                    $Description = "补单成功!";
                } else {
                    $Description = "补单失败!";
    ...
    private function WxQueryOrder($out_trade_no) {
        ...
        $list = http_request(C('SERVER_HOST') . "Payment/Payments/QueryWxOrder", $data, $head);
        $list = json_decode($list, true);
        $trade_state = $list['Data']['WxPayData']['m_values']['trade_state'];
        return $trade_state;
    }

    private function StaffWxQueryOrder($out_trade_no) {
    ...
    private function AddWxOrder($out_trade_no) {

        $data = array("Out_trade_no" => $out_trade_no, "SystemUserSysNo" => session('SysNO'));
        $url = C('SERVER_HOST') . "POS/WXSupplyOrder";
        $data = json_encode($data);
        $head = array("Content-Type:application/json;charset=UTF-8", "Content-length:" . strlen($data), "X-Ywkj-Authentication:" . strlen($data));
        $list = http_request($url, $data, $head);
        $list = json_decode($list, true);
