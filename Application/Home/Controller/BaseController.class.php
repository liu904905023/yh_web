        ...
        if ($flag == 1) {
            ...
            if (!$list) {
                ...
                foreach ($list['model'] as $row => $val) {
                    if (($val['URL'] != "/Order/platform_order_search") && ($val['URL'] != "/Order/order_search_alipay") && ($val['URL'] != "/OrderFund/orderfund?Top=1") && ($val['URL'] != "/OrderFund/orderfund") && ($val['URL'] != "/OrderExtend/order_extend") && ($val['URL'] != "/Refund/refund") && ($val['URL'] != "/OrderStatistics/order_statistics")&&($val['URL'] != "/OrderExtendRecharge/order_extend_recharge")&&($val['URL'] != "/BusinessRefund/refund")) {
                        $info[$row]['SysNo'] = $val['SysNo'];
                        $info[$row]['RoleName'] = $val['RoleName'];
                        $info[$row]['Description'] = $val['Description'];
                        $info[$row]['URL'] = $val['URL'];
